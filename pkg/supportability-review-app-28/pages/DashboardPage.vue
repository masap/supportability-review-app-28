...
export default {
  ...
  async fetch() {
    ...
    if (
      ...
      const srSchema = this.$store.getters["management/schemaFor"](
        SUPPORTABILITY_REVIEW_CRD_IDS.REVIEW_BUNDLE,
      );
      ...
      const isSROperatorNotInstalledOnApps =
        ...
        !installedApps?.find(
          (item) =>
            item.id.includes("rancher-supportability-review") &&
            !item.id.includes("rancher-supportability-review-crd"),
        );
